...
#   /MFDNN_code/EEG/EEG_CP.py
# =========================================================
CURRENT_FILE = Path(__file__).resolve()
EEG_DIR = CURRENT_FILE.parent
...
def select_cp_hyperparameters(X_train, y_train, rank_values, reg_values, val_ratio=0.25):
    X_tr, X_val, y_tr, y_val = train_test_split(
        X_train, y_train, test_size=val_ratio, random_state=756
    )
...
for run_idx in range(frun):
    ...
    X_train = np.transpose(EEG_x[:, :, train_idx], (2, 0, 1))
    ...
    best_hyper, best_model = select_cp_hyperparameters(
        X_train, y_train, rank_values, reg_values
    )
    ...
    print(
        f"Run {run_idx + 1}: Best rank={best_hyper['rank']}, "
        f"reg={best_hyper['regularization']}, Test F1={f1:.4f}"
    )
...
save_path = RESULTS_DIR / "EEG_CP.csv"
