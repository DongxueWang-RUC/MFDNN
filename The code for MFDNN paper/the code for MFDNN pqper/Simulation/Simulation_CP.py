...
SIMULATION_DIR = CURRENT_FILE.parent
...
def CP_regression(X_train, y_train, X_test, rank, regularization=0.01):
    X_train_r = np.transpose(X_train, (1, 0, 2, 3))
...
def select_tensor_hyperparameters(X_train, y_train, val_fraction=0.25, random_state=756):
    ...
    train_idx, val_idx = train_test_split(
        indices, test_size=val_fraction, random_state=random_state
    )
...
def _process_single_config(args):
    ...
    try:
        Xlist = np.load(data_path / f"Xlist_T{T}_n{n}.npy", allow_pickle=True)
        ylist = np.load(data_path / f"ylist_T{T}_n{n}.npy", allow_pickle=True)
...
def run_cp_parallel(n_jobs=None, data_path=DATA_DIR):
    if n_jobs is None:
...
def main():
    ...
    cp_results = run_cp_parallel(n_jobs, DATA_DIR)
    ...
    save_path = RESULTS_DIR / "Simulation_CP.csv"
